'use client'

import pb from '@/db/db'
import { useState } from 'react'

export const NewsLetter = () => {
  const [mail, setMail] = useState('')

  const subscribe = async (e: any) => {
    e.preventDefault()
    const record = await pb.collection('news_letter').create({ mail })
    console.log(record)
  }

  return (
    <section className='py-14 max-w-screen-xl w-full mx-auto'>
      <div className='relative overflow-hidden mx-4 px-4 py-14 rounded-2xl bg-blue-600 md:px-8 md:mx-8'>
        <div className='relative z-10 max-w-xl mx-auto sm:text-center'>
          <div className='space-y-3'>
            <h3 className='text-3xl text-white font-bold'>
              Subscribe to our newsletter
            </h3>
            <p className='text-blue-100 leading-relaxed'>
              Stay up to date with the roadmap progress, announcements
              and exclusive discounts feel free to sign up with your
              email.
            </p>
          </div>
          <div className='mt-6'>
            <form
              noValidate
              onSubmit={(e) => subscribe(e)}
              className='flex items-center justify-center bg-white rounded-lg p-1 sm:max-w-md sm:mx-auto'>
              <input
                type='email'
                value={mail}
                onChange={(e) => setMail(e.target.value)}
                placeholder='Enter your email'
                className='text-gray-500 w-full p-2 outline-none'
              />
              <input
                type='submit'
                value='  Subscribe'
                className='p-2 px-3 rounded-lg font-medium text-white bg-blue-600 hover:bg-blue-500 active:bg-blue-700 duration-150 outline-none shadow-md focus:shadow-none sm:px-4'
              />
            </form>
            <p className='mt-3 max-w-lg text-[15px] text-blue-100 sm:mx-auto'>
              No spam ever, we are care about the protection of your
              data. Read our{' '}
              <a className='underline' href='javascript:void(0)'>
                {' '}
                Privacy Policy{' '}
              </a>
            </p>
          </div>
        </div>
        <div
          className='absolute inset-0 w-full h-full'
          style={{
            background:
              'linear-gradient(268.24deg, rgba(59, 130, 246, 0.76) 50%, rgba(59, 130, 246, 0.545528) 80.61%, rgba(55, 48, 163, 0) 117.35%)'
          }}></div>
      </div>
    </section>
  )
}
